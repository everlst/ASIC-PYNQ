module fc_control_unit (  //似乎会把1024位输入当成IO口导致FF剧增
  input  wire        clk_i,
  input  wire        rst_n_i,
  ...
  // 提供给fc_top的输入地址（由fc_top产生，我们这里只是连接）
  input  wire [1:0]    fc_in_addr_i
);
  ...
  // FIFO读控制：在推理状态下且需要数据时读取
 
  assign fifo_rd_en = (current_state == S_PREFETCH) && !fifo_empty && !cache_ready;
   always @(posedge clk_i or negedge rst_n_i) begin
    if (!rst_n_i) begin
    rd_en_d1 <= 0;
    end else  begin
  ...
  reg  [1:0] fc_in_addr_i_d[0:1];
  always @(posedge clk_i or negedge rst_n_i) begin
    if (!rst_n_i) begin
    fc_in_addr_i_d[0] <= 2'd0;
    fc_in_addr_i_d[1] <= 2'd0;
    end else  begin
    fc_in_addr_i_d[0] <= fc_in_addr_i;
    fc_in_addr_i_d[1] <= fc_in_addr_i_d[0];
  end
 end
  // ===== 输入数据处理 =====
  ...
  // ===== 输出数据处理 =====
  reg [7:0] output_buffer [0:161];  // 128(FC1) + 32(FC2) + 2(FC3) = 162字节
  reg [7:0] output_addr;
  reg       output_en;
  reg       valid_d1;
  // 捕获推理结果
  always @(posedge clk_i or negedge rst_n_i) begin
    if (!rst_n_i) begin
      ...
      output_addr <= output_addr + 1;  // 重置地址，为下次推理准备
      valid_d1 <= 1;
    end else if (valid_d1) begin
      output_addr <= 8'b0;  // 重置地址，为下次推理准备
      valid_d1 <= 0;
    end
  end
